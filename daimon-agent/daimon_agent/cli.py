"""daimon-agent CLI — practical use of the wrapper: interactive chat and agentic run.

  daimon-agent chat --model Qwen/Qwen3.5-4B --checkpoint doubter.pt --quantization nf4
  daimon-agent run  --model … --checkpoint … --stdtools "what is 17*23?"

Without --checkpoint — the bare base model (for comparison). --stdtools wires in calculator +
knowledge_base (demo tools) for agentic mode. Console-script: `daimon-agent` (see pyproject [project.scripts]).
"""
...
def _build_agent(args):
    """args -> a ready MetaAgent. Lazy-import of the meta-attention library (GPU path)."""
    import json
    _resolve_from_run_dir(args)
    if not args.model:
        raise SystemExit("--model (HF id of the base model) or --run-dir with run.json is required")
    from meta_attention import MetaAttentionConfig  # lazy
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="daimon-agent",
...
def main(argv: Optional[list] = None) -> None:
    ...
    if sys.platform == "win32":   # #1: --help with non-ASCII (→) crashed with UnicodeEncodeError on a cp1251 console
        try:
            ...
            pass
    try:
        from meta_attention.quiet import quiet_transformers
