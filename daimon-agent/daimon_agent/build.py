...
`MetaAgent` (for `ChatLoop` or `.run(task)`). The CLI `daimon-agent chat/run` is built on this.

The seam is split into two levels: `assemble_agent` (from a READY backend+tokenizer — tested on
FakeBackend without GPU) and `build_agent` (loads a real model via the meta-attention library; lazy-imports it).
"""
...
def assemble_agent(backend, tokenizer, *, tools=None, system: Optional[str] = None,
    ...
    Low-level seam — independent of the meta-attention library, tested on FakeBackend without GPU.
    """
    reg = _registry(tools)
...
def build_agent(config, checkpoint: Optional[str] = None, *, tools=None,
    ...
    Lazy-import of the meta-attention library (GPU path) — `import daimon_agent` without a model stays lightweight.
    """
    from .backends import DaimonBackend  # lazy: pulls in meta_attention
