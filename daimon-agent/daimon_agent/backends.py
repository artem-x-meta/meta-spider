...
Decouples daimon-agent from the engine: the loop knows only Policy, Policy knows only
backend.generate(prompt) -> InferenceOutput. That is why daimon-agent is independent of the
training toolkit (daimon-loom) and holds no copy of the two-pass mechanism (meta-attention) — each
backend lazily pulls in ITS OWN heavy dependency.

  DaimonBackend — in-process GPU, lazy import of the meta-attention pipeline (research)
  LlamaCppBackend   — CPU prod, calls the llama.cpp fork (already independent)   [stub]
  OpenAIBackend     — remote: daimon-agent as a client to an OpenAI-compatible
...
class DaimonBackend:
    """In-process two-pass inference through the meta-attention pipeline (GPU/research path).

    The meta_attention import is lazy (inside from_daimon) — the package stays
    self-contained: it depends ONLY on the thin meta-attention library, not on the training toolkit
    (daimon-loom).
    """
    ...
    def generate(self, prompt: str) -> InferenceOutput:
        ...
        # to the meta-attention library + GPU validation: does the readout correlate with actual refusal?).
        # For now — text.
        return InferenceOutput(text, signal=None)

    @classmethod
    def from_daimon(cls, config, checkpoint: Optional[str] = None, **kwargs):
        from meta_attention import MetaAttentionPipeline, Doubter  # lazy
        pipeline = MetaAttentionPipeline.from_pretrained(config)
        if checkpoint:
...
class StopBackend:
    ...
    CRITICAL for the native tool format ([daimon_agent.native]): without truncation the model
...
class LlamaCppBackend:
    ...
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            ...
            "(see project-llamacpp-deploy). Use DaimonBackend for now.")
...
class OpenAIBackend:
    """Remote shim: daimon-agent as a client to an OpenAI-compatible server that
    ...
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            ...
            "(polish phase). Use DaimonBackend for now.")
