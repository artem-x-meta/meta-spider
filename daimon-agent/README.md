...
> Agentic runtime + chat + OpenAI-compatible serving for the **two-pass meta-attention** wrapper.
> The voice's latent decision → a standard action vocabulary.

A component of the **Daimon** project:

| Component | Role |
|---|---|
| **meta-attention** | the library — two-pass mechanics + checkpoint contract (separate repo) |
| **daimon-voices** | the voices (Doubter, GoalAnchor, ChronoAnchor) + the `Voice` contract |
| **daimon-loom** | training + benchmark-evaluation pipeline |
| **daimon-agent** (here) | agentic sessions + chat + serving |

daimon-agent depends on the **meta-attention** library (inference), not on daimon-loom (training).

## Practical usage

Chat or an agentic run with your own wrapper, in one command (console-script `daimon-agent`):

```bash
# interactive chat with an attached Doubter
daimon-agent chat --model Qwen/Qwen3.5-4B --checkpoint doubter.pt --quantization nf4
# one agentic task (--stdtools = calculator + knowledge_base; without --checkpoint — bare base)
daimon-agent run  --model Qwen/Qwen3.5-4B --checkpoint doubter.pt --stdtools "what is 17*23?"
```

Or from Python — one builder assembles the whole stack (backend + native tool format + `StopBackend` + policy):

```python
from daimon import DaimonConfig
from daimon_agent import build_agent, ChatLoop

cfg = DaimonConfig(model_name="Qwen/Qwen3.5-4B", quantization="nf4")
...
**emitted token**. The two-pass Daimon wrapper breaks it:

```
Standard engine:   prompt → generate() → tokens (decision visible in text / tool_call)
Daimon:       prompt → Pass 1 (hidden) → encoder → cognitive tokens
...
- `LatentActionRenderer` — reads the decision from a latent `signal` (`{"refuse": bool}` or
  ...
  **Half of the seam (the consumer) is ready.** What's left is emitting the signal from the core: the
  ...
  part of `ActionTokenAdapter` (a meta-attention-library change + GPU validation). See `DaimonBackend`.
...
| `backends.py` | `InferenceBackend` — the inference **seam** (Daimon / llama.cpp / OpenAI) |
...
**Two seams** keep daimon-agent away from everything heavy:
...
- `InferenceBackend` (prompt → text + latent signal) — we don't know *how* inference is computed:
  `DaimonBackend` (in-process GPU), `LlamaCppBackend` (CPU prod), `OpenAIBackend` (remote). Each
  lazily pulls its own dependency.

So the engine is deterministic and tested on CPU without a GPU (`pytest tests/`), and the
**meta-attention** library is imported lazily only in `DaimonBackend`. The dependency on
daimon-loom (the training toolkit) is zero.

## Example

```python
from daimon_agent import MetaAgent, Tool, ToolRegistry, BackendPolicy, FakeBackend
...
# real wrapper (needs the meta-attention library + a Doubter checkpoint)
# from daimon_agent import DaimonPolicy
# from daimon import DaimonConfig
# policy = DaimonPolicy(DaimonConfig(...), checkpoint="doubter.pt")
...
`RuleSolverPolicy` instead of a model; in prod → `BackendPolicy(DaimonBackend(...))`):
...
```python
from daimon_agent import build_react_agent, ToolRegistry, calculator, knowledge_base, GemmaFormat
...
For a trained 12B: `backend = DaimonBackend.from_daimon(cfg, checkpoint="doubter.pt")` —
loop/prompt/parsing are the same.

## Status

`0.4.0` — **full agent runtime**: loop + real tools + native tool format (`native.py`) +
two seams (action/inference, incl. `LatentActionRenderer`) + chat + tau-bench-lite + an
**OpenAI-compatible server** (`serve.py`: `/v1/chat/completions` plus latent side-channels
`/v1/meta/goal` and `/v1/meta/gain`; two legs — HF pipeline and patched llama.cpp). Tests green.
Next: per-input signal readout from the library (enables the latent path end-to-end).
