...
Runs the dataset through the frozen base once and caches:
  ...
  - pass1_text — what the model produced without voices (Pass 1 baseline)
...
class DatasetSample:
    ...
    Attributes:
        ...
        pass1_text: what the model produced without voices.
...
class ActivationDatasetCollector:
    """Collect activations + Pass 1 answers for training the voice.

    Args:
        pipeline: MetaAttentionPipeline WITHOUT attached voices (clean base).
    ...
    def _capture_cut_hidden_batch(
        ...
        from daimon_loom.training.losses import build_correction_target
