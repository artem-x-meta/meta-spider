"""Trainer — two-pass training loop for voices.
...
One training step = two forwards of the same model + backward on top:
    ...
    Pass 1 (read, cached):
      ...
      cognitive_tokens ← voice.encoder(activation_list)
      voice.buffer.fill(cognitive_tokens)

    Pass 2 (write + loss):
      logits, loss ← base.forward(input_ids+target, labels=labels_with_prompt_mask)
                     # the voice's CA hooks see buffer.is_filled and inject
...
class Trainer:
    """Two-pass trainer for the Doubter (or another CA-based voice).

    Args:
        voice: a Doubter already attached to the pipeline (via `pipeline.attach`).
    ...
    def __init__(
        ...
        if voice not in pipeline.injectors:
            raise ValueError("Voice must be attached to the pipeline before training.")
        self.voice = voice
    ...
    def build_optimizer(self) -> AdamW:
        ...
        if encoder is None or not isinstance(encoder, nn.Module):
            raise RuntimeError("Voice.encoder is not initialized (call pipeline.attach).")
        ...
        for epoch in range(cfg.epochs):
            ...
            def _opt_step():
                ...
                torch.nn.utils.clip_grad_norm_(
                    voice.get_trainable_parameters(), cfg.grad_clip,
                )
            ...
            history["ca_gate_map"].append(voice.get_ca_gate_map())
            ...
            if val_loss is not None and val_loss < best_val_loss:
                ...
                if checkpoint_dir:
                    ...
                    voice.save_checkpoint(str(Path(checkpoint_dir) / "best_model.pt"))
    ...
    def _train_step(
        ...
        try:
            if cut is not None:
                ...
                from daimon_loom.slice_forward import slice_last_hidden
    ...
    def save_checkpoint(self, path: str, epoch: Optional[int] = None) -> None:
        """Save the voice state_dict + the current epoch."""
        payload = {
            "format_version": "1.0",
            "epoch": epoch,
            "encoder_state": (self.voice.encoder.state_dict()
                              if isinstance(self.voice.encoder, nn.Module) else None),
            "ca_state": {k: m.state_dict() for k, m in self.voice.ca_modules.items()},
        }
        torch.save(payload, path)

    def load_checkpoint(self, path: str) -> None:
        """Load the state_dict into the voice (encoder + CA)."""
        state = torch.load(path, weights_only=False, map_location="cpu")
        if isinstance(self.voice.encoder, nn.Module) and state.get("encoder_state"):
            self.voice.encoder.load_state_dict(state["encoder_state"])
        for k, st in state.get("ca_state", {}).items():
            if k in self.voice.ca_modules:
                self.voice.ca_modules[k].load_state_dict(st)
