...
def _suite_exclude(suite_path: Optional[str]) -> set:
    ...
    if not suite_path:
        return set()
    from daimon_loom.data.agentic_mix import _norm
...
def build_universal_stage(
    ...
    *,
    ...
    if mix is None:
        from daimon_loom.data.agentic_mix import build_training_mix
    ...
    (rd / "README.md").write_text(
        ...
        f"- load with `meta_attention` (see run.json); the `gain` knob tunes caution at inference.\n",
        encoding="utf-8")
    ...
    if eval_suite and suite_path:
        ...
        from daimon_loom.evaluation.agentic_suite import compare_base_vs_doubter
    ...
    # 8) optional GGUF sidecar (llama.cpp / CPU / edge) — needs daimon-deploy installed
    if export_gguf:
        try:
            from daimon_deploy.export import export_from_run_dir
            gguf = export_from_run_dir(run_dir, verbose=verbose)
            C.mark_artifact(gguf)
            if verbose:
                print(f"  GGUF sidecar → {gguf}", flush=True)
        except ImportError:
            print("  [warn] --export-gguf needs daimon-deploy installed (`pip install -e daimon-deploy`); "
                  "skipped.", flush=True)
...
def add_args(p) -> None:
    ...
    p.add_argument("--export-gguf", action="store_true",
                   help="also export a llama.cpp GGUF sidecar (needs daimon-deploy installed)")
