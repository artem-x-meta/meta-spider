"""Watchdog — read-only сенсор неуверенности (одна из «ног» meta-spider).

В отличие от Doubter (CA-инъекция в residual), Watchdog НИЧЕГО не впрыскивает: он только ЧИТАЕТ
когнитивный токен в точке решения через обученный энкодер и прогоняет через линейную пробу →
P(uncertain). Скор используется, чтобы ГЕЙТИТЬ внешнее действие (docs-lookup / refuse / escalate) —
без побочной порчи генерации.

Валидация: cog-probe @ L32 slot, in-domain AUC ~0.715, cog > сырой активации; read-only gated lookup
+1/0-потерь на агентном ODEX (`docs/results/qwen-14b/cogprobe-watchdog.md`). Это «watchdog-нога»
из meta-spider-scope (дрейф/неуверенность как отдельный read-only канал, не модулятор).
"""
