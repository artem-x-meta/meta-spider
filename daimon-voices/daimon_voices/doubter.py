...
from daimon_voices.config import DoubterConfig
from meta_attention.injection import BottleneckCrossAttention
from meta_attention.encoders.base import Encoder
from meta_attention.encoders.multi_token import MultiTokenEncoder
...
class Doubter(Voice):
    ...
    def on_attach(self, pipeline: "MetaAttentionPipeline") -> None:
        ...
        if pipeline.config.hidden_dim is None or pipeline.config.num_layers is None:
            raise RuntimeError(
                "MetaAttentionConfig.resolve_defaults() must be called before attach. "
                "Usually MetaAttentionPipeline.from_pretrained() does this."
            )
    ...
    def _install_ca_hooks(self, pipeline: "MetaAttentionPipeline") -> None:
    ...
    def save_checkpoint(self, path: str) -> None:
        ...
        if self.encoder is None:
            raise RuntimeError("Nothing to save — the voice is not attached to a pipeline.")
    ...
    def _load_pending_state(self) -> None:
        ...
        if exp_cross and list(exp_cross) != list(self._cross_attn_layers):
            raise RuntimeError(
                ...
                f"{list(self._cross_attn_layers)}. Set the same layers in MetaAttentionConfig "
        ...
        if exp_target and list(exp_target) != list(getattr(self, "_target_layers", []) or []):
            raise RuntimeError(
                ...
                f"MetaAttentionConfig (usually via the run.json manifest).")
