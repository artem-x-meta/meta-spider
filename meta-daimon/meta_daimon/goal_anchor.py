"""GoalAnchor — the goal-drift Watchdog LEG: a behavior modifier against goal drift.

NOT `meta_core.watchdog.Watchdog` (that one is a confidence SENSOR in the Doubter stack whose
probe gates pointwise injection / external actions). GoalAnchor is a full injection modifier
...
from meta_daimon.config import GoalAnchorConfig
...
class GoalAnchor(Modifier):
    ...
    def on_attach(self, pipeline: "MetaSpiderPipeline") -> None:
        ...
        if pipeline.config.hidden_dim is None or pipeline.config.num_layers is None:
            raise RuntimeError("MetaSpiderConfig.resolve_defaults() must run before attach "
                               "(MetaSpiderPipeline.from_pretrained does this).")
    ...
    def _install_hooks(self, pipeline: "MetaSpiderPipeline") -> None:
    ...
    def save_checkpoint(self, path: str) -> None:
        if self.encoder is None:
            raise RuntimeError("Nothing to save — the modifier is not attached.")
        cfg = self.config
        torch.save({
            "format_version": "1.1",
            "kind": "goal_anchor",
