...
Usage (local):
  python offline_curve.py --mode cossim \
      ...
      --framework-path daimon-framework
...
def run_behavioral(args, meta, results: Path):
    ...
    from daimon_loom.data.dataset import check_answer_correctness
...
def main():
    ...
    ap.add_argument("--framework-path", default=None, help="(vestigial — meta_attention/daimon_loom are installed editable)")
    ap.add_argument("--dataset-loader-path", default=None, help="(vestigial — the native loader lives in daimon_loom.data)")
