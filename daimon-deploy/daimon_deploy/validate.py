...
The encoder is imported directly from `meta_attention` (editable install); --framework-path is only a
...
def _import_multi_token(framework_path: Optional[str]):
    try:
        ...
        if framework_path:
            # framework_path = the meta-attention repo root (or a tree containing it)
            sys.path.insert(0, str((Path(framework_path) / "meta-attention").resolve()))
