...
The fourth Daimon component. Takes a trained Doubter wrapper (encoder + BottleneckCA,
...
```text
daimon-voices ──provides──> trained doubter_checkpoint.pt (format: meta-attention checkpoint contract)
                              │
                   metadeploy export
                              ▼
        doubter_sidecar.gguf  +  ggml/C++ forward (the meta-attention library, cpp/)
                              ▼
...
```bash
pip install -e daimon-framework-dev/daimon-deploy        # numpy + gguf (export side)
# torch is only needed to read the trained .pt (usually already present with the meta-attention library)
```
...
# 3. llama.cpp fork with meta-injection (see the meta-attention library, cpp/llama_patch/) + two-pass run
...
(one model load for many prompts). Build the fork with the library's `cpp/build_llama.sh --backend {cuda,cpu,metal}`.
The llama.cpp patch, the ggml forward and the how-to live in the [meta-attention library](https://github.com/artem-x-meta/meta-attention)
(`cpp/`, `cpp/llama_patch/`). Helper here: `scripts/run_meta_generate.sh`.

## Layout

| | what |
|---|---|
| `daimon_deploy/export.py` | checkpoint.pt → GGUF sidecar (library API + `metadeploy export`) |
| `daimon_deploy/spec.py` | numpy reference forward of the encoder — **the pinned spec for ggml** |
| `daimon_deploy/validate.py` | numpy-spec vs PyTorch check (`metadeploy validate`) |
...
- **C++ encoder — ✅ validated against PyTorch** (diff ~1e-6): both multi_token (Gemma-2B) and
  **selective** — the current encoder of all Doubters (Qwen/Granite/Gemma-12B), max abs diff **8.9e-7**,
  ...
  Build: the library's `cpp/build.sh` (direct g++; its `CMakeLists.txt` is the canonical dependency description).
...
- **End-to-end injection into llama.cpp generation — ✅ WORKS (turnkey).** The llama.cpp fork
  (the library's `cpp/llama_patch/`) loads the sidecar as a meta-adapter, runs the built-in ggml encoder forward
