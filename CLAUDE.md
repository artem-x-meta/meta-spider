...
## Packages (four, one job each — dependency graph)

```
meta-core   inference primitives (frozen base + wrapper, hooks, encoder, gated CA, checkpoint contract)
            → depends on NOTHING. Production inference installs only this.
meta-agent  agentic runtime + native tool-use            → meta-core
meta-loom   training + eval + the build-universal factory → meta-core + meta-agent
meta-deploy export to a GGUF sidecar for llama.cpp (CPU)  → meta-core
meta_spider umbrella shim: `from meta_spider import *` re-exports meta-core + meta-loom (back-compat)
```
...
```bash
# install (editable, from this folder). Order matters: core → agent → loom → umbrella.
...
- **Watchdog** (`meta_core.watchdog`): read-only — a `ConfidenceProbe` on the cognitive token gates an
  external action WITHOUT injecting into generation.
...
- Publishing: framework → Codeberg (`codeberg.org/imperius/meta-spider`, push from the mirror clone via
  native git); per-model wrappers → HuggingFace (`HF_HUB_ENABLE_HF_TRANSFER=1`, per-file for big files).
