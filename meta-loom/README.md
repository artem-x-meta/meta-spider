...
Training + evaluation for the Meta-Spider wrapper. Depends on **meta-core** (+ **meta-agent** for
agentic eval).
...
`run.json` is self-contained: `metaloom train/eval --run-dir` and `meta-agent run --run-dir` read
everything from it.
...
| API | what |
|---|---|
| `BaselineComparison(pipeline, QABenchmark(...)).run()` | selective QA eval: base vs modifier, with significance |
| `AgentComparison(pipeline, doubter=...).run([AgentTask(...)])` | agentic eval (multi-step tool use) via meta-agent |
