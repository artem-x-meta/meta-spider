"""Agentic eval: an HONEST base vs Doubter comparison in a single loop via Meta-Agent.

**Loom uses the Agent.** meta_loom used to carry its own `AgenticBenchmark` — a manual ReAct loop
(regex parsing + `history += "Observation:"`) that doesn't work on live instruct models (the model
appends a fake Observation, there are no stop tokens). Here — delegation to `meta_agent`: the real
...
from meta_agent import (
    BackendPolicy, MetaAgent, MetaSpiderBackend, NativeToolPrompt, NativeToolRenderer,
...
class AgentComparison:
    ...
    Args:
        pipeline: MetaSpiderPipeline (carries tokenizer + generate).
    ...
    def _run_arm(self, task: AgentTask, use_doubter: bool) -> tuple[bool, int, bool]:
        ...
        be = StopBackend(MetaSpiderBackend(self.pipeline, max_new_tokens=self.max_tokens,
