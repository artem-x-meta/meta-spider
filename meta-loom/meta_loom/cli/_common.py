...
def build_meta_config(manifest: dict, *, device: Optional[str] = None,
    ...
    """MetaSpiderConfig from the manifest (device-auto). max_memory — Part B (offload)."""
...
def resolve_dataset_loader(loader_path: Optional[str] = None):
    ...
    By default — the package's NATIVE loader (`meta_loom.data.dataset`): a random dev needs
